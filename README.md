...
- [Installing it as an app (PWA / APK)](#installing-it-as-an-app-pwa--apk)
...
## Installing it as an app (PWA / APK)

The client is a full **Progressive Web App**: installable from the browser, runs standalone with no address bar, and shows an offline page instead of a dinosaur.

```bash
cd client
npm run build && npm start     # a service worker only registers in production
```

Then in Chrome: **⋮ → Install Blink**, or use the in-app prompt. On iPhone: **Share → Add to Home Screen**.

### Turning it into an APK

Because it is a real PWA, it can be packaged into a signed Android app **without writing any Android code** — the PWA is wrapped in a Trusted Web Activity:

1. Deploy the client over **HTTPS** (Render/Vercel both give you this free).
2. Go to [pwabuilder.com](https://www.pwabuilder.com), paste the URL, and hit **Package for stores → Android**.
3. It produces a signed `.apk`/`.aab` plus an `assetlinks.json`.
4. Put that file at `client/public/.well-known/assetlinks.json` and redeploy. Without it Android cannot verify you own the domain, and the app launches with a browser address bar across the top — the one thing wrapping it was meant to remove.

### What the service worker does, and deliberately does not do

Hand-written rather than generated by `next-pwa` or similar. A chat app is close to the worst case for naive caching — the usual plugins will happily cache an API response or a stale bundle and leave you debugging a ghost. The rules are narrow on purpose:

| Request | Strategy | Why |
|---|---|---|
| The API and the socket | **Never intercepted** | Cached chat is worse than no chat |
| Anything cross-origin | **Never intercepted** | The API, the WebSocket and every uploaded photo live on another origin, so one check keeps the worker away from all of them — and away from caching someone's photos into a shared browser cache |
| `/_next/static/*` | Cache-first | Content-hashed: the URL changes when the bytes do, so a hit is always fresh |
| Page navigations | Network-first → offline page | A deploy is picked up immediately; offline still shows something |

Two details that are easy to get wrong and both matter:

**Maskable icons are a separate set, not a resize.** Android crops a maskable icon into a circle/squircle/teardrop and only guarantees the middle 80%. The `any` icons let the logo fill the square; the `maskable` ones give it far more padding. Shipping only `any` icons is why so many PWAs have their edges sliced off on Android.

**`apple-mobile-web-app-capable` is set by hand.** Next 15 now emits only the standardised `mobile-web-app-capable`, which iOS Safari does not read. Without the legacy Apple-prefixed tag, an installed app on iPhone launches inside Safari chrome with an address bar — exactly what installing it was supposed to remove.

The service worker is **not** registered in development. One caching a dev bundle is a special kind of misery: you change a file, nothing happens, and you spend an hour blaming your code.
